...
const ProductOverview = ({
  ...
  return (
    <>
      <div className="border-grey-20 px-6 flex justify-end border-t border-b py-2.5">
    ...
    <div className="p-base rounded-rounded hover:bg-grey-5 group flex-col">
      <div className="relative">
        ...
        <Link to={`${product.id}`}>
          {product.thumbnail ? (
            <img
              className="rounded-rounded block min-h-[100px] object-cover"
              src={product.thumbnail}
            />
          ) : (
            <div className="bg-grey-5 rounded-rounded flex min-h-[100px] items-center justify-center">
              <ImagePlaceholder />
            </div>
          )}
          <div>
