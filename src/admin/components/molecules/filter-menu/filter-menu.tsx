  ...
  return (
    <Popover {...props}>
      <Popover.Trigger data-filter-component={true} asChild>
        ...
        <button
          className={clsx(
            "h-full rounded-rounded focus-visible:shadow-input focus-visible:border-violet-60 flex items-center space-x-1 focus-visible:outline-none"
          )}
        >
          <div className="rounded-rounded bg-grey-5 border-grey-20 inter-small-semibold flex h-6 items-center border px-2">
            ...
            <div className="rounded bg-grey-5 ml-1 flex items-center p-1">
      ...
      >
        {options?.map((opt, index) => {
          return (
            <SelectCheckItem
              checked={
                Array.isArray(value) && typeof opt.value === "string"
                  ? value.includes(opt.value)
                  : false
              }
              onCheckedChange={() => {
                if (Array.isArray(value) && typeof opt.value === "string") {
